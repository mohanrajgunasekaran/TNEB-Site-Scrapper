...
class TNEBScraper:
    ...
    def extract_details(self, consumer_no: str) -> ResultRow:
        ...
        if self.captcha_rejected(body_text):
            ...
            return ResultRow(consumer_no=consumer_no, info="INVALID_CAPTCHA")
        ...
        while attempt <= self.max_retries:
            try:
                ...
                if result.info == "INVALID_CAPTCHA":
                    logging.warning("CAPTCHA rejected for consumer %s", consumer_no)
...
def export_to_excel(rows: Sequence[ResultRow], output_path: str) -> Path:
    ...
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        ...
        for col, width in {"A": 18, "B": 28, "C": 45, "D": 18, "E": 16, "F": 30}.items():
            ws.column_dimensions[col].width = width

    return out


def save_checkpoint(rows: Sequence[ResultRow], checkpoint_path: Path) -> None:
    checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
    with checkpoint_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Consumer No", "Consumer Name", "Consumer Address", "Bill Amount (Rs)", "Due Date", "Info"])
        for row in rows:
            writer.writerow([row.consumer_no, row.consumer_name, row.consumer_address, row.bill_amount_rs, row.due_date, row.info])


def load_checkpoint(checkpoint_path: Path) -> List[ResultRow]:
    ...
    with checkpoint_path.open("r", newline="", encoding="utf-8") as f:
        ...
        for r in reader:
            rows.append(
                ResultRow(
                    consumer_no=r.get("Consumer No", "") or "",
                    consumer_name=r.get("Consumer Name", "") or "",
                    consumer_address=r.get("Consumer Address", "") or "",
                    bill_amount_rs=r.get("Bill Amount (Rs)", "") or "",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--start", required=True, help="Start consumer number")
    parser.add_argument("--end", required=True, help="End consumer number")
    parser.add_argument("--keywords", nargs="*", default=[], help="Optional consumer name keywords")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    existing_rows = load_checkpoint(checkpoint_path) if args.resume else []
    processed = {r.consumer_no for r in existing_rows}

    driver = None
    results: List[ResultRow] = list(existing_rows)
    ...
    try:
        ...
        for consumer_no in generate_consumer_numbers(args.start, args.end):
            if consumer_no in processed:
                logging.info("Skipping already processed consumer %s", consumer_no)
                continue
        ...
        export_to_excel(results, output_path)
        save_checkpoint(results, checkpoint_path)
        logging.info("Exported %s rows to %s", len(results), output_path)
        return 0
